...
internal static class Crowded
{
        ...
        public static void Prefix(CreateOptionsPicker __instance)
        {
            ...
            if (GameStates.IsVanillaServer)
            {
                if (GameOptionsManager.Instance.GameHostOptions != null &&
                    GameOptionsManager.Instance.GameHostOptions.MaxPlayers > 15)
    ...
    {
        [SuppressMessage("ReSharper", "UnusedMember.Global")]
        public static bool Prefix(CreateOptionsPicker __instance) => __instance.mode != SettingsMode.Host;
    }
...
public class AbstractPagingBehaviour(IntPtr ptr) : MonoBehaviour(ptr)
{
    ...
    {
        bool chatOpen = HudManager.Instance.Chat.IsOpenOrOpening;
        bool menuOpen = HudManager.Instance.GameMenu.IsOpen;
        ...
        if (Input.touchSupported)
        {
            foreach (Touch touch in Input.touches)
            {
                ...
                if (chatOpen || menuOpen) break;
                if (touch.deltaPosition.y > 0f) { Cycle(false); break; }
                if (touch.deltaPosition.y < 0f) { Cycle(true); break; }
            }
        }

        if (!chatOpen && !menuOpen && (Input.GetKeyDown(KeyCode.UpArrow) || Input.GetKeyDown(KeyCode.LeftArrow) || Input.mouseScrollDelta.y > 0f))
            Cycle(false);
        else if (!chatOpen && !menuOpen && (Input.GetKeyDown(KeyCode.DownArrow) || Input.GetKeyDown(KeyCode.RightArrow) || Input.mouseScrollDelta.y < 0f))
            Cycle(true);
    }
    ...
    {
        PageIndex = Mathf.Clamp(PageIndex + (increment ? 1 : -1), 0, MaxPageIndex);
    }
}
...
public class MeetingHudPagingBehaviour(IntPtr ptr) : AbstractPagingBehaviour(ptr)
{
    ...
    {
        base.Update();
        if (meetingHud.state is MeetingHud.VoteStates.Animating or MeetingHud.VoteStates.Proceeding) return;
        if (meetingHud.TimerText.text.Contains($" ({PageIndex + 1}/{MaxPageIndex + 1})")) return;
    ...
    public override void OnPageChanged()
    {
        ...
        foreach (PlayerVoteArea button in Targets)
        {
            if (i >= PageIndex * MaxPerPage && i < (PageIndex + 1) * MaxPerPage)
            {
                ...
                int rel = i % MaxPerPage;
                int row = rel / 3;
                int col = rel % 3;
                button.transform.localPosition = meetingHud.VoteOrigin + new Vector3(
        ...
        foreach (ShapeshifterPanel panel in Targets)
        {
            if (i >= PageIndex * MaxPerPage && i < (PageIndex + 1) * MaxPerPage)
            {
                ...
                panel.transform.localPosition = new(
                    shapeshifterMinigame.XStart + shapeshifterMinigame.XOffset * (rel % 3),
                    shapeshifterMinigame.YStart + shapeshifterMinigame.YOffset * (rel / 3),
    ...
    public override void OnPageChanged()
    {
        ...
        foreach (VitalsPanel panel in Targets)
        {
            if (i >= PageIndex * MaxPerPage && i < (PageIndex + 1) * MaxPerPage)
            {
                ...
                int rel = i % MaxPerPage;
                panel.transform.localPosition = new(
                    vitalsMinigame.XStart + vitalsMinigame.XOffset * (rel % 3),
                    vitalsMinigame.YStart + vitalsMinigame.YOffset * (rel / 3),
                    panel.transform.localPosition.z);
            }
            else panel.gameObject.SetActive(false);
            i++;
