...
// Adapted from: https://github.com/0xDrMoe/TownofHost-Enhanced/blob/main/Patches/CrowdedPatch.cs
...
internal static class Crowded
{
    ...
    public static class CreateOptionsPickerRefresh
    {
        public static bool Prefix(CreateOptionsPicker __instance)
        {
            ...
            __instance.GameModeText.text = DestroyableSingleton<TranslationController>.Instance.GetString(
                GameModesHelpers.ModeToName[GameOptionsManager.Instance.CurrentGameOptions.GameMode]);
            return false;
        ...
        public static bool Prefix(CreateOptionsPicker __instance, [HarmonyArgument(0)] IGameOptions opts)
        {
            ...
            for (var i = 1; i < __instance.MaxPlayerButtons.Count - 1; i++)
            {
                __instance.MaxPlayerButtons[i].enabled =
                    __instance.MaxPlayerButtons[i].GetComponentInChildren<TextMeshPro>().text == selectedAsString;
            }
    ...
    public static class NormalGameOptionsAreInvalid
    {
        [SuppressMessage("ReSharper", "UnusedMember.Global")]
        public static bool Prefix(NormalGameOptionsV10 __instance, ref bool __result)
        {
            ...
            if (GameStates.IsVanillaServer && __instance.MaxPlayers > 15)
                __result = true;
    ...
    public static class BeforeHostGamePatch
    {
        ...
        public static void Postfix()
        {
            Logger.Info("Host Game finished is being called!", "Crowded");
        }
    }
}
...
public class AbstractPagingBehaviour : MonoBehaviour
{
    ...
    {
        var chatIsOpen = HudManager.Instance.Chat.IsOpenOrOpening;
    ...
    /// <summary>
    /// Loops around if you go over the limits.
...
public class MeetingHudPagingBehaviour : AbstractPagingBehaviour
{
    ...
    public override void Update()
    {
        ...
        if (meetingHud.TimerText.text.Contains($" ({PageIndex + 1}/{MaxPageIndex + 1})"))
            return;
